...
from word_detection.model.lstm import LSTMWakeWord
from word_detection.utils.dataset import WakeWordData, collate_fn


def save_checkpoint(
    checkpoint_path, model, optimizer, scheduler, model_params, notes=None
):
    torch.save(
        {
            "notes": notes,
...
def test(test_loader, model, device, epoch):
    ...
    with torch.no_grad():
        for idx, (mfcc, label) in enumerate(test_loader):
            ...
            accs.append(acc)
            print(
                "Iter: {}/{}, accuracy: {}".format(idx, len(test_loader), acc),
                end="\r",
            )
...
def main(args):
    use_cuda = not args.no_cuda and torch.backends.mps.is_available()
    torch.manual_seed(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sample_rate", type=int, default=8000, help="sample_rate for data"
    )
    ...
    parser.add_argument(
        "--eval_batch_size", type=int, default=32, help="size of batch"
    )
